...
clusters_t* cluster(int original_num_clusters, int desired_num_clusters, int* final_num_clusters, int num_dimensions, int num_events, float* fcs_data_by_event) {
    ...
    #pragma omp parallel shared(clusters,fcs_data_by_event,fcs_data_by_dimension,shared_likelihoods,likelihood,old_likelihood,ideal_num_clusters,min_rissanen,regroup_iterations) 
    {
        ...
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.N), sizeof(float)*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.pi),sizeof(float)*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.constant),sizeof(float)*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.avgvar),sizeof(float)*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.means),sizeof(float)*num_dimensions*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.R),sizeof(float)*num_dimensions*num_dimensions*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.Rinv),sizeof(float)*num_dimensions*num_dimensions*original_num_clusters));
        CUDA_SAFE_CALL(cudaMallocManaged(&(temp_clusters.memberships),sizeof(float)*my_num_events*(original_num_clusters+NUM_CLUSTERS_PER_BLOCK-original_num_clusters % NUM_CLUSTERS_PER_BLOCK)));
       
        // Allocate a struct on the device 
        clusters_t* u_clusters;
        CUDA_SAFE_CALL(cudaMallocManaged(&u_clusters, sizeof(clusters_t)));
        DEBUG("Finished allocating memory on device for clusters.\n");
        
        // Copy Cluster data to device
        u_clusters = &temp_clusters;
        ...
        float* u_fcs_data_by_event;
        float* u_fcs_data_by_dimension;
        
        // allocate and copy relavant FCS data to device.
        int mem_size = num_dimensions * my_num_events * sizeof(float);
        CUDA_SAFE_CALL(cudaMallocManaged(&u_fcs_data_by_event, mem_size));
        CUDA_SAFE_CALL(cudaMallocManaged(&u_fcs_data_by_dimension, mem_size));

        u_fcs_data_by_event = &fcs_data_by_event[num_dimensions*events_per_gpu*tid];
        ...
        u_fcs_data_by_dimension = temp_fcs_data;
        ...
        #pragma omp master
        {
            ...
            seed_clusters<<< 1, NUM_THREADS_MSTEP >>>( u_fcs_data_by_event, u_clusters, num_dimensions, original_num_clusters, my_num_events);
            cudaDeviceSynchronize();
            ...
            constants_kernel<<<original_num_clusters, NUM_THREADS_MSTEP>>>(u_clusters, original_num_clusters,num_dimensions);
            ...
            clusters[0].avgvar = temp_clusters.avgvar;
            ...
            clusters[0].means = temp_clusters.means;
            ...
            clusters[0].Rinv = temp_clusters.Rinv;
        ...
        temp_clusters.avgvar = clusters[0].avgvar;
        ...
        temp_clusters.means = clusters[0].means;
        ...
        temp_clusters.Rinv = clusters[0].Rinv;
        ...
        float* u_likelihoods;
        PRINT("TEST B");
        cudaMallocManaged(&u_likelihoods, sizeof(float)*NUM_BLOCKS);
        PRINT("TEST C");
        ...
        startTimer(timers.memcpy);
        PRINT("TEST3");
        cudaMallocManaged( &u_c, sizeof(float));
        PRINT("TEST4");
        stopTimer(timers.memcpy);
        PRINT("TEST5");
        ...
        for(int num_clusters=original_num_clusters; num_clusters >= stop_number; num_clusters--) {
            ...
            estep1<<<dim3(num_clusters,NUM_BLOCKS), NUM_THREADS_ESTEP>>>(u_fcs_data_by_dimension,u_clusters,num_dimensions,my_num_events);
            estep2<<<NUM_BLOCKS, NUM_THREADS_ESTEP>>>(u_fcs_data_by_dimension,u_clusters,num_dimensions,num_clusters,my_num_events,u_likelihoods);
            cudaDeviceSynchronize();
            #pragma omp master
            {
                regroup_iterations++;
            ...
            startTimer(timers.memcpy);
            shared_likelihoods[tid*NUM_BLOCKS] = *u_likelihoods;
            ...
            while(iters < MIN_ITERS || (fabs(change) > epsilon && iters < MAX_ITERS)) {
                ...
                mstep_N<<<num_clusters, NUM_THREADS_MSTEP>>>(u_clusters,num_dimensions,num_clusters,my_num_events);
                cudaDeviceSynchronize();
                stopTimer(timers.m_step);
                startTimer(timers.memcpy);
                clusters[tid].N = temp_clusters.N;
                ...
                mstep_means<<<gridDim1, NUM_THREADS_MSTEP>>>(u_fcs_data_by_dimension,u_clusters,num_dimensions,num_clusters,my_num_events);
                cudaDeviceSynchronize();
                stopTimer(timers.m_step);
                startTimer(timers.memcpy);
                clusters[tid].means=temp_clusters.means;
                ...
                mstep_covariance2<<<dim3((num_clusters+NUM_CLUSTERS_PER_BLOCK-1)/NUM_CLUSTERS_PER_BLOCK,num_dimensions*(num_dimensions+1)/2), NUM_THREADS_MSTEP>>>(u_fcs_data_by_dimension,u_clusters,num_dimensions,num_clusters,my_num_events);
                cudaDeviceSynchronize();
                stopTimer(timers.m_step);
                startTimer(timers.memcpy);
                clusters[tid].R = temp_clusters.R;
                ...
                startTimer(timers.constants);
                constants_kernel<<<num_clusters, NUM_THREADS_MSTEP>>>(u_clusters,num_clusters,num_dimensions);
                cudaDeviceSynchronize();
                clusters[tid].constant = temp_clusters.constant;
            ...
            clusters[tid].Rinv = temp_clusters.Rinv;
            ...
            temp_memberships = temp_clusters.memberships;
            ...
            if(num_clusters > stop_number) {
                ...
                stopTimer(timers.cpu);
                startTimer(timers.memcpy);
                temp_clusters.memberships=temp_memberships;
        ...
        // cleanup GPU memory
        CUDA_SAFE_CALL(cudaFree(u_likelihoods));
     
        CUDA_SAFE_CALL(cudaFree(u_fcs_data_by_event));
        CUDA_SAFE_CALL(cudaFree(u_fcs_data_by_dimension));
        ...
        CUDA_SAFE_CALL(cudaFree(u_clusters));
